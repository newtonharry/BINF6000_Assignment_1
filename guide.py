...
import os, math, numpy, urllib.request, urllib.parse, urllib.error, urllib.request, urllib.error, urllib.parse
...
def search(query, format='list', dbName='uniprotkb', size=None, columns=""):
    """
    ...
    size: max number of results (specify None for all results)
    ...
    if dbName.startswith('uniprot'):
        # Construct URL
        if format=='list':
            if size == None:  # no limit to number of results returned
                url = "{}{}/stream?format={}&query={}".format(__uniprotUrl__, dbName, format,
                                                            urllib.parse.quote(query, safe='"'))
            else:
                url = "{}{}/stream?format={}&query={}&size={}".format(__uniprotUrl__, dbName, format, 
                                                            urllib.parse.quote(query, safe='"'), str(size))
...
def searchSequences(query, database='uniprotkb'):
    """ Search for sequences matching the given query in the given database
    (must be 'uniprot'), and return a list of sequence IDs. """
    ids = search(query, size = None)
    return ids

def getSequence(query, id=None, dbName = 'uniprotkb', alphabet = Protein_Alphabet, format = 'fasta'):
    ...
    if query:
        url = "{}{}/stream?format={}&query={}".format(__uniprotUrl__, dbName, format, query)
    elif id:
        url = "{}{}/stream?format={}&query=accession:{}".format(__uniprotUrl__, dbName, format, query)
